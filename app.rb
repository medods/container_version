...
get '/list/:release' do
  params['release'] ||= "v39"
  services = @@connection.exec "SELECT * FROM container_versions WHERE release_version = \'#{params['release']}\'"
...
post '/update' do
  ...
  service = @@connection.exec "SELECT * FROM container_versions \
                              WHERE application_name = \'#{params['service']}\' AND release_version = \'#{params['release']}\' \
                              LIMIT 1" 
